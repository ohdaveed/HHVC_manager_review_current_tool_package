...
function button(label, kind = 'primary', target = null) {
  ...
  const onclick = target ? ` onclick="renderPage('${target}'); return false;"` : ''
  ...
  return `<div class="cards">${cards
    .map((c) => {
      ...
      const attr = c.url
        ...
        : ` onclick="${c.target ? `renderPage('${c.target}'); return false;` : 'return false;'}"`
...
function exportCurrentManagerReviewCsv() {
  ...
  managerReviewDownload(
    `${snapshot.page_key}-manager-review.csv`,
    managerReviewToCsv(rows),
...
function exportAllPageDecisionTemplateCsv() {
  ...
  managerReviewDownload(
    'hhvc-all-page-manager-review-template.csv',
    managerReviewToCsv(rows),
    'text/csv;charset=utf-8'
  )
