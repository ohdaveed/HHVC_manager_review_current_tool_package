...
- **Known open question, deliberately left to Task 3:** whether `--legacy-danger-*` reads acceptably for `editor` in dark mode. The contrast test answers it; if the ΔE against `--legacy-warning-*` is too small, the fallback is to keep `editor` on `success` and give `action` the teal, moving the new family to a different slot. Task 3's measurement decides, not a preference.
  - **Resolved in implementation, and the fallback was rejected rather than taken** (2026-08-23, on review): `--legacy-danger-*` measured acceptably, so `editor` took it and `action` took the green it vacated. The fallback as written would have put an editor-only "do not publish" note on the one colour that reads as approval — the exact swap this task exists to undo — so it should not have been offered even conditionally. If the measurement had gone the other way, the answer was a different non-approval family, never `success`.
