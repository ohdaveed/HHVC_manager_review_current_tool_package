...
| Railway Postgres       | Managed Postgres              | Production review-state store; Railway injects `DATABASE_URL`, which selects the Postgres driver. `DATA_DB_PATH` (SQLite) is the local-dev and CI fallback | Bearer `REVIEW_API_TOKEN` (+ optional principals) | Medium when sync is used      | `build_scripts/storage.js`, `.claude/skills/verify-railway-backend/`      |
