...
- **Corpus definition is separate from ingestion on purpose**:
  ...
  change: **78 documents, 813 chunks** (re-measured 2026-08-16) — `hhvc-policy`
  430, `mockup-draft` 233, `karl` 96, `sfgov-live` 28, `sfgov-style` 24,
  `sfds` 2. `karl` rose from 53 when `docs/karl-export-field-map.md` was added;
  `sfds` from 1 as its capture grew.
- **Retrieval is still brute-force cosine in JS.** 813 chunks ranks in
