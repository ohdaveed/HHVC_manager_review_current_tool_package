...
sources that both looked (`U5`). Those need a decision from Digital Services, not another capture.

`U12` and `U16` are a third and narrower case, and it is worth not collapsing them back into the
second. Their source conflict IS settled — the live form beat the Help Center in both. What keeps
them open is that each was settled with E1 evidence answering a question E1 cannot reach: whether a
Related entry of that type actually publishes, and whether a Topic template renders a second
Spotlight. Those need E4, not a decision and not another form capture.
...
### Precedence, decided 2026-08-15: the live admin capture is the source of truth

**Where sources disagree about what an editor form contains, E1 wins** — over the Help Center (E3),
over the older repo docs, and over the `karl` notes in `pages/*.js`. That directive is what narrowed
`U12` and `U16` and demoted `O3` and `O9` from live conflicts to plain documentation errors. Narrowed
rather than closed, for the reason the next paragraph gives: it settles what the FORM contains, which
is not what either of those two actually asked.

**It does not reach E4, because E4 answers a different question.** E1 says what fields the editor
form has; E4 says what a published page actually renders. A `Related` entry has no description field
_and_ publishes the destination's title only — those are two findings about two different things,
and the second was measured on live sf.gov, not read off a form. The card-inheritance buckets below
rest entirely on E4 and are **not** superseded by anything in the E1 capture. Do not delete them as
outdated; there is no E1 evidence that could replace them, because the editor form cannot tell you
...
| 2   | **`Button link` block**      | Transaction `section_specifics`; Campaign/Topic/Agency/Report Spotlight; Topic `Content` section content        | radio `SF.gov page` → page chooser               | radio `External URL` → URL text field | "Link text" \* + "Screenreader label". Link text **maxlength 255** (E1); the Help Center's "can only be 25 characters" is style guidance, not a schema limit — see below |
...
**The Topic cap conflicts with the E1 capture** on the authoring half only: the capture recorded
Topic's `Spotlight` as a repeatable block inside `content_fields`, so a second one can be added on
the form. Whether the published template renders it is a separate question the form cannot answer,
and `U16` stays open on exactly that. The mockup is within either reading today.
...
| Special cases                                   | `special_cases`          | no                           | single                        | plain text — heading override for the two panels below                                                    | —                                                                                              |
...
| Redirect this page to                           | `redirect_url`           | —                            | single                        | plain text, **disabled by design**                                                                        | — never                                                                                        |
...
| Spotlight 1              | `spotlight_1`                | not recorded                           | **single** (max 1 per field, 2 total) | one type: `Spotlight`, no chooser                                                              | `section` with `component: 'spotlight'`                         |
...
| `seoTitle` / `metaDescription`                    | all 29 pages                        | Promote tab `seo_title` / `search_description` — **verified against the live form 2026-08-15** (`U11` closed)                                                                                                                         |
...
| `U6`  | open     | **`Primary agency` is required on 7 types (every type but Agency) and the mockup has no field for it.** Also `Part of` (Information) and `good_for_community` (Transaction). A page cannot be saved without Primary agency.                                                                                                                                                                                                                                                                                                                                                                             | Add a mockup field, or accept it as an at-build-time input |
...
| `U12` | narrowed | **Narrowed 2026-08-15 by precedence; half of it is still open.** The Help Center states Related accepts only Transaction/Information/Campaign/Topic; the live Campaign picker returned and accepted a Resource Collection page. Under the source-of-truth rule the live form wins, so the documented four-type restriction is wrong as a statement about the FORM (`O3`). What that does not establish is that such an entry saves and publishes: the evidence is a **picker** observation, and a chooser offering an option is not the CMS accepting it. Disproved for authoring, unproven for output. | E4 — save and publish a Resource Collection into Related   |
...
| `U16` | narrowed | **Narrowed 2026-08-15; the half that was asked is still open.** Topic's `content_fields` chooser lists `Spotlight` among six block types in a repeatable stream, so a second one can be ADDED on the form, and the Help Center's "Topic allows 1" loses as a claim about the form. But `U16` asked whether the template RENDERS a second one, and E1 cannot answer that — see the precedence section: E1 says what the form has, E4 says what publishes. `healthyHousingTopic` uses one either way, so nothing in the mockup depends on it.                                                             | E4 — publish a Topic carrying two Spotlights               |
...
| `O3`  | _Related is restricted to 4 content types_                                                                                | Karl Help Center (unchanged upstream)                                                          | Contradicted by Campaign's live picker at E1 — wrong about the form; publish behaviour open as `U12`                                                                                        |
...
| `O14` | _"Buttons can only be 25 characters long"_ | Karl Help Center, Button component (upstream) | The live `Button link` field carries **`maxlength="255"`**, measured 2026-08-15 on a Transaction `section_specifics` block. The 25 is real guidance and worth following — the same page ships a verb-first button-text library — but it is **not enforced by the form**, so a long label saves rather than failing. Ten mockup labels exceeded it; all ten were shortened on guidance rather than on constraint (`U19`). |
...
rather than through Karl MCP. The four-type restriction text is byte-for-byte unchanged, so this is a
standing documented-policy-versus-live-form conflict rather than a stale-docs problem — the live form
wins on what the picker offers, and `U12` carries the part the picker cannot answer.
...
- **Live Karl admin, captured 2026-08-15** (E1) — an authenticated, read-only session against
  ...
  narrowed `U12` and `U16` to their published-output halves, and drove the 27-note rewrite recorded
  above.
...
`U16`). Closing `U13` and confirming `U8`'s remaining panels therefore needed an authenticated
browser session against `New: Agency`, `New: About us` and `New: Report` — the same method that
produced the E1 and E2 captures. That session has since happened: the 2026-08-15 capture recorded
all three forms and closed both. This paragraph is kept as the record of why the sweep alone was not
enough, not as an outstanding task.
