...
`pages/*.js`. This reverses the rule recorded here on 2026-08-15, and the reversal is a directive,
...
| `U24` | closed   | **Closed 2026-08-23, the same day it opened.** `article11Guide`'s page-level Spotlight button read "View Health Code Article 11" — **27 characters** — the one label in the corpus over the 25-character cap that the 2026-08-23 precedence reversal (`O14`) turned from guidance into a rule. It is now **"View Article 11"**, 15 characters (`pages/health-code-article-11.js:22`). The words dropped are carried by the block they sit in: the Spotlight's own title reads "Read the full Health Code" and its paragraph names the municipal code, so "Health Code" was printed twice within one card. Re-measured after the change across all 18 `button` values in the corpus: none exceeds the cap, and the longest is `mosquitoWorkshop`'s 25-character "Request a workshop online", at it exactly. `U19` had reported the corpus clean because it swept section- and step-level buttons only and never reached the page-level Spotlight; this is the sweep that reached it. | closed                                                     |
