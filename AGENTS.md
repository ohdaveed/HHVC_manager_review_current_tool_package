...
- **Page inventory (46 pages):** Canonical order and keys live in `js/page-data.js`. Page module paths and load order live in **`build_scripts/page-files.js`** (single source for `validate.js`, `extract-pages.js`, and `sync-tracking-sheet.js`). When adding a page, update `page-files.js`, `js/page-data.js`, and matching `<script>` tags in `index.html` — `bun run validate` checks index.html stays in sync.
