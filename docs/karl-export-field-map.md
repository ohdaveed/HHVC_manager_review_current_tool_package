...
| `U19` | open | **Nine button labels still exceed SF.gov's 25-character button-text guidance.** 41 and 40 characters on `article11Guide`, 31 on `insectsReport`, 30 and 28 on `article11Guide`, and 27 on four pages sharing "View Health Code Article 11". Not a schema violation (`O14`) — the form accepts 255 — but the Help Center gives a verb-first library these could draw on. `payFee`'s was shortened; these are copy decisions, one per page. | Content decision |
---
...
| `O14` | _"Buttons can only be 25 characters long"_ | Karl Help Center, Button component (upstream) | The live `Button link` field carries **`maxlength="255"`**, measured 2026-08-15 on a Transaction `section_specifics` block. The 25 is real guidance and worth following — the same page ships a verb-first button-text library — but it is **not enforced by the form**, so a long label saves rather than failing. Ten mockup labels exceed it; one (`payFee`) was shortened, the rest are recorded under `U19`. |
