...
- **A question E1 cannot reach** (`U12`, `U16`). Their source conflict is settled too, the live
  form beating the Help Center in both, but each was settled with form evidence answering a
  question about PUBLISHED OUTPUT: whether a Related entry of that type actually publishes, and
  whether a Topic template renders a second Spotlight. Those need E4 — not a decision, and not
  another form capture.
...
`U12` and `U16` and demoted `O3` and `O9` from live conflicts to plain documentation errors.
...
type requires exactly `title`, `primary_agency` and `slug`** (along with `cost` on Transaction, and
`services_title` and `resources_title` on Agency) — nothing else — with one exception:
...
**The documented caps are not enforced by the forms, measured 2026-08-15.** Campaign's
`spotlight_1` accepted **two** Spotlight blocks on its own, with its add control still enabled —
so "up to 2" is a rendering or editorial limit, not a schema one, exactly like the 25-character
button cap (`O14`). The same conflict was already recorded for Topic as `U16`. Treat this table as
guidance about what a template will show, and the per-type tables below as what the form accepts.
...
| section-level `button`/`buttonUrl` outside a step | 8 Transaction, 4 Report, 2 Campaign, 1 Topic                    | Campaign's sit inside `spotlight` sections and are fine. The other 12 are `U1`                                                                                                                                                        |
...
| `U16` | narrowed | **Narrowed 2026-08-15; the half that was asked is still open.** Topic's `content_fields` chooser lists `Spotlight` among six block types in a repeatable stream, so a second one can be ADDED on the form — and the 2026-08-15 Campaign measurement recorded above shows the same is true of `spotlight_1`, so the documented caps are editorial rather than schema. But `U16` asked whether the TEMPLATE renders a second one, and E1 cannot answer that. `healthyHousingTopic` uses one either way.                                                                                                   | E4 — publish a Topic carrying two Spotlights               |
